...
> Toutes les requêtes ciblent **SQL Server (T-SQL)**. La base se crée avec
> `migration.sql` puis se remplit avec `seed.sql` (voir `docker-compose.yml` à la racine).
...
(Attention aux divisions entières : `AVG` sur un type entier, et `total / nombre`.)

---

## 1.7 — Produits jamais évalués

Listez les produits **actifs** qui n'ont reçu **aucun avis** (aucune ligne dans `reviews`),
alors qu'ils sont en stock (`inventory.quantity` strictement positif).
...
Utilisez une CTE récursive pour parcourir l'arborescence.
